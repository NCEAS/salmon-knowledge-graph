...
SAR_ENDPOINT = "https://knb.ecoinformatics.org/knb/streamnet/SAR/"
...
SAR_RESOURCE = Namespace(f"{SAR_ENDPOINT}lod/resource/")
...
class SAR_ONT(DefinedNamespace):
    ...
    _NS = Namespace(f"{SAR_ENDPOINT}lod/ontology/")
...
("SAR", "salmonCharacteristic.SAR"),
("RETURNSMISSING", "salmonCharacteristic.RETURNSMISSING"),
("REARINGTYPE", "salmonCharacteristic.REARINGTYPE"),
("TSO", "salmonCharacteristic.TSO"),
("TAR", "salmonCharacteristic.TAR"),
("RETURNDEF", "salmonCharacteristic.RETURNDEF"),
("HARVESTADJ", "salmonCharacteristic.HARVESTADJ"),
("OCEANHARVEST", "salmonCharacteristic.OCEANHARVEST"),
("MAINSTEMHARVEST", "salmonCharacteristic.MAINSTEMHARVEST"),
("TRIBHARVEST", "salmonCharacteristic.TRIBHARVEST"),
("BROODSTOCKREMOVED", "salmonCharacteristic.BROODSTOCKREMOVED")
}
...
def graphify(df: DataFrame) -> Graph:
    ...
    for _, row in df.iterrows():
        ...
        population_uri = SAR_RESOURCE[f"population.{population_id}"]
        p = RDF.type
        o = SAR_ONT.SalmonPopulation
        ...
        smoltLocation_iri = STREAMNETRES[f"smoltLocation.{id}"]
        p = RDF.type
        o = STREAMNETONT.SmoltLocation
        graph.add((smoltLocation_iri, p, o))
        p = RDF.type
        o = GEO.Feature
        graph.add((smoltLocation_iri, p, o))

        if (~pd.isnull(row["SMOLTLOCPTCODE"])):
            p = SAR_ONT.smoltLocationPTCode
            o = Literal(row["SMOLTLOCPTCODE"], datatype=XSD.string)
            graph.add((smoltLocation_iri, p, o))

        adultLocation_iri = STREAMNETRES[f"adultLocation.{id}"]
        p = RDF.type
        o = STREAMNETONT.AdultLocation
        graph.add((adultLocation_iri, p, o))
        p = RDF.type
        o = GEO.Feature
        graph.add((adultLocation_iri, p, o))

        p = FO.associatedRegion
        graph.add((smoltLocation_iri, p, recoveryDomain_uri))
        graph.add((adultLocation_iri, p, recoveryDomain_uri))

        observation_collection_uri = SAR_RESOURCE[f"observationCollection.{id}"]
        p = RDF.type
        o = SAR_ONT.Escapement_ObservationCollection
        ...
        graph.add((observation_collection_uri, p, smoltLocation_iri))
        graph.add((observation_collection_uri, p, adultLocation_iri))
        ...
        for characteristic in SALMON_MEASUREMENT_CHARACTERISTIC:
            ...
            if (pd.isnull(measurement_value)):
                ...
                observation_uri = SAR_RESOURCE[f"observation.{measurement_variable}.{id}"]
                p = RDF.type
                o = SAR_ONT.Escapement_Observation
                ...
                if (~pd.isnull(row["SARTYPE"])):
                    p = SAR_ONT.sarType
                    o = Literal(row["SARTYPE"], datatype=XSD.string)
                    graph.add((observation_uri, p, o))

                if (~pd.isnull(row["SCOPEOFINFERENCE"])):
                    p = SAR_ONT.scopeOfInference
                    o = Literal(row["SCOPEOFINFERENCE"], datatype=XSD.string)
                    graph.add((observation_uri, p, o))

                if (~pd.isnull(row["SMOLTDEF"])):
                    p = SAR_ONT.smoltDefinition
                    o = Literal(row["SMOLTDEF"], datatype=XSD.string)
                    graph.add((observation_uri, p, o))
                ...
                measurement_uri = SAR_RESOURCE[f"measurement.{measurement_variable}.{id}"]
                p = RDF.type
                o = SAR_ONT.Escapement_Measurement
                graph.add((measurement_uri, p, o))
                # p = OBOE.measuresCharacteristic
                # o = SAR_RESOURCE[f"salmonCharacteristic.SARIJ"]
                # graph.add((measurement_uri, p, o))
                ...
                p = OBOE.measuresCharacteristic
                o =  SAR_ONT[measurement_named_individual]
                graph.add((measurement_uri, p, o))
        ...
        #add metadata
        dataset_name = "SAR"
        description = "For natural origin fish this is the point estimate of the number of returning natural origin adults, divided by the point estimate of the number of smolts that produced those returning adults. This value is multiplied by 100 to obtain a percentage."
        title = "Smolt to adult return rate (SAR)"
...
if __name__ == "__main__":
    ...
    df = pd.read_csv("../datasets/streamnet-cai/Streamnet_SAR.csv", header=0, encoding='latin-1')
    output_folder = "output"

    os.makedirs(output_folder, exist_ok=True)

    graph = graphify(df)
    destination = os.path.join(output_folder, f"streamnet_sar-dataset.ttl")
